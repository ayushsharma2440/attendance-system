...
const createtable = (sql)=>{
    ...
    sql.query("CREATE TABLE IF NOT EXISTS attendance ( attendance_id INT AUTO_INCREMENT PRIMARY KEY,user_id INT ,date DATE NOT NULL, at_time TIME,location_id INT,FOREIGN KEY (user_id) REFERENCES USER(id),FOREIGN KEY (location_id) REFERENCES locations(location_id))",(err,result)=>{
        if (err) throw err;
        else console.log("Table Location created");
    });
